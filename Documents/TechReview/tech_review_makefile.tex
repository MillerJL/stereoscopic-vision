...
\tab \tab {\Medium\textbf{f. Best Option:   }}Due to time constraints and previous experience a team member has with the technology, React with Electron is the best choice for creating the desktop application user interface. Using React will allow the developers on the team to avoid unnecessary work migrating components between the desktop and mobile applications. \newline \tab \tab \tab option.\\
\vspace{5mm}
...
\tab {\Medium\textbf{10. State Management System for Applications (Authorship: John Miller)}}\\
\tab \tab {\Medium\textbf{a. Options:  }}Redux\\
\tab \tab {\Medium\textbf{b. Goals for use: }}Need a system to easily and consistently manage applicaiton state across both \newline \tab \tab \tab the desktop and mobile applications.\\
\tab \tab {\Medium\textbf{c. Criteria being evaluated:   }}Ease of use for developer and community support.\\
\tab \tab {\Medium\textbf{d. Comparison of technologies: }}N/A \\
\tab \tab {\Medium\textbf{e. Discussion:  }}Redux is a library that implements Facebook’s Flux design pattern [38]. It is
\newline \tab \tab \tab composed of three main parts: actions, dispatchers, and reducers. Dispatchers send actions
\newline \tab \tab \tab to reducers where they are matched with a certain function to execute. The function intakes
\newline \tab \tab \tab the current state of the application and returns a deep copied state with whatever necessary
\newline \tab \tab \tab changes made to it. It is immutable so the entire state of the application can be viewed on a
\newline \tab \tab \tab “timeline”, e.g. one can literally follow everything that happens in your application step by
\newline \tab \tab \tab step and see with 100\% certainty what everything in the application looked like at each of
\newline \tab \tab \tab those steps. Not only does this make debugging simple, it also removes all the overhead of
\newline \tab \tab \tab the developer tracking the state of all components of the application across the entire project.
\newline \tab \tab \tab This technology works across both React and React-Native[38] and provides an incredibly
\newline \tab \tab \tab simple way to manage the application states consistently across both applications.\\
\tab \tab {\Medium\textbf{f. Best Option:   }} Redux\\
\vspace{5mm}

\tab {\Medium\textbf{11. User Interface Library (Authorship: John Miller)}}\\
\tab \tab {\Medium\textbf{a. Options:  }}Material-UI\\
\tab \tab {\Medium\textbf{b. Goals for use: }}The applications require a library that will give a consistent and functional user
\newline \tab \tab \tab interface across both applications \newline \tab \tab \tab the desktop and mobile applications.\\
\tab \tab {\Medium\textbf{c. Criteria being evaluated:   }}Ease of use for developer, community support, options for both
\newline \tab \tab \tab mobile and desktop applications\\
\tab \tab {\Medium\textbf{d. Comparison of technologies: }}N/A \\
\tab \tab {\Medium\textbf{e. Discussion:  }}Material-UI is a user interface framework that follows Google’s Material Design
\newline \tab \tab \tab specification [39]. It is available for both React and React-Native [39], with only minimal
\newline \tab \tab \tab differences between the two. Because it follows such a well established design pattern, the
\newline \tab \tab \tab team can spend more time focusing on building a functional user interface. Both the React
\newline \tab \tab \tab and React-Native implementations are component based, meaning they are predesigned
\newline \tab \tab \tab drag/droppable components that look good together and offer various configuration options.
\newline \tab \tab \tab The library offers all the basic tools needed to interact with the user: buttons, navigation,
\newline \tab \tab \tab loading animations, messages, visual feedback, etc. Not only will the use of this library save
\newline \tab \tab \tab developer time, it will help create a functional, clean, and consistent application.\\
\tab \tab {\Medium\textbf{f. Best Option:   }} Material-UI\\
\vspace{5mm}
