...
int main(int, char**)
{
	...
	system("ffmpeg -i left.mp4 > output1.txt 2>&1");
	system("ffmpeg -i right.mp4 > output2.txt 2>&1"); 
	...
	if (right_time > left_time) {
		...
		strs << difference;
		string s = strs.str();
		string command = "ffmpeg -i right.mp4 -ss " + s + " RC.mp4";
		system(command.c_str());
		system("ffmpeg -i left.mp4 -vf scale=960:540 output1.mp4");
	...
	if (left_time > right_time) {
		...
		strs << difference;
		string s = strs.str();
		string command = "ffmpeg -i left.mp4 -ss " + s + " LC.mp4";
		system(command.c_str());
		system("ffmpeg -i LC.mp4 -vf scale=960:540 output1.mp4");
		system("ffmpeg -i right.mp4 -vf scale=960:540 output2.mp4");
		system("rm -f LC.mp4");
	}
	if (left_time == right_time) {
		difference = 0;
	...
	system("ffmpeg -i output1.mp4 -i output2.mp4 -filter_complex \"[0:v:0]pad=iw*2:ih[bg]; [bg][1:v:0]overlay=w\" finalVideo.mp4");
